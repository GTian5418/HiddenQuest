...
```
app/
├─ src/main/java/com/oshin/hiderecent/
│  ├─ Main.kt                # 模块入口（XposedModule），分发 system / launcher 两条 hook；隐藏名单的读取与缓存
...
│  ├─ PrefsProvider.kt       # ContentProvider，把隐藏名单暴露给其他进程读取
...
读取通道按优先级依次尝试（`Main.readHiddenPackages()`）：

| 级别 | 通道 | 依赖模块进程？ | 说明 |
|---|---|---|---|
| 1 | ContentProvider（`com.oshin.hiderecent.prefs`） | 是 | 模块活着时最准，永远反映最新勾选 |
...
- **写入即推送。** 模块 UI 每次改动都会广播 `com.oshin.hiderecent.PREFS_CHANGED`，
  extra `hide_list` 带完整名单。桌面进程里的动态接收器收到后零 IPC 直接落盘 + 刷新内存缓存。
  桌面进程是常驻的，所以这条通道最稳；这也是「杀进程后仍生效」的关键。

另外还有一条进程内缓存（TTL 1.5s），避免每次划动最近任务都走一遍 Binder/IPC。
